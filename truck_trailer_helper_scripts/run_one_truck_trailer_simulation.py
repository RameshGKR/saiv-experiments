...
def run_one_simulation(coord, hoek, variables, run_counter):
    T_end=70.0
    ocp = Ocp(T=T_end)
    N = 100
    M = 1
    ...
    ocp.add_objective(ocp.integral(c1*((x1-x1_tf)**2+(y1-y1_tf)**2)*(c2*((ocp.t/T_end)**c3))))
    ocp.add_objective(ocp.integral(t1*((theta1-theta1_tf)**2+(theta0-theta0_tf)**2)*(t2*((ocp.t/T_end)**t3))))
    ...
    for k in range(Nsim):
        ...
        with open('truck_trailer_hillrun_'+str(run_counter)+'.csv', "a+") as output_file:
